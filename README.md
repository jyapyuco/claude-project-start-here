...
.claude/memory/           Project memory: index, decisions, gotchas, conventions, journal
```
...
**Tier 1 — project memory** (`.claude/memory/`, committed): decisions made and why, gotchas discovered, conventions learned, plus a `journal.md` with one digest line per session. `CLAUDE.md` imports the index (`MEMORY.md`) so every session starts already knowing what past sessions learned.
...
```
session ──ends──> dream.ps1 ──claude -p (haiku)──> .claude/memory/*   (project facts)
